...
FuturePlan = namedtuple(
    'FuturePlan', ['lataccel', 'roll_lataccel', 'v_ego', 'a_ego'])
...
class LataccelTokenizer:
    def __init__(self):
        ...
        self.bins = np.linspace(
            LATACCEL_RANGE[0], LATACCEL_RANGE[1], self.vocab_size)
...
class TinyPhysicsSimulator:
    def __init__(self, model: TinyPhysicsModel, data_path: str, debug: bool = False) -> None:
    ...
    def reset(self) -> None:
        ...
        self.current_lataccel_history = [x[1]
                                         for x in state_target_futureplans]
    ...
    def sim_step(self, step_idx: int) -> None:
        ...
        pred = np.clip(pred, self.current_lataccel - MAX_ACC_DELTA,
                       self.current_lataccel + MAX_ACC_DELTA)
        if step_idx >= CONTROL_START_IDX:
            self.current_lataccel = pred
        else:
            self.current_lataccel = self.get_state_target_futureplan(step_idx)[
                1]

        self.current_lataccel_history.append(self.current_lataccel)

    def control_step(self, step_idx: int, control_input) -> None:
    ...
    def get_state_target_futureplan(self, step_idx: int) -> Tuple[State, float, FuturePlan]:
        ...
        return (
            State(roll_lataccel=state['roll_lataccel'],
                  v_ego=state['v_ego'], a_ego=state['a_ego']),
            state['target_lataccel'],
            FuturePlan(
                lataccel=self.data['target_lataccel'].values[step_idx +
                                                             1:step_idx + FUTURE_PLAN_STEPS].tolist(),
                roll_lataccel=self.data['roll_lataccel'].values[step_idx +
                                                                1:step_idx + FUTURE_PLAN_STEPS].tolist(),
                v_ego=self.data['v_ego'].values[step_idx +
                                                1:step_idx + FUTURE_PLAN_STEPS].tolist(),
                a_ego=self.data['a_ego'].values[step_idx +
                                                1:step_idx + FUTURE_PLAN_STEPS].tolist()
            )
        )

    def pre_step(self):
        state, target, futureplan = self.get_state_target_futureplan(
            self.step_idx)
    ...
    def step(self, control_input) -> None:
        self.control_step(self.step_idx, control_input)
        self.sim_step(self.step_idx)
        cost = self.compute_cost(last_step_only=True)
        res = cost, self.target_lataccel_history[
            self.step_idx], self.current_lataccel, self.state_history[self.step_idx], self.futureplan
        self.step_idx += 1
        return res
    ...
    def plot_data(self, ax, lines, axis_labels, title) -> None:
        ...
        ax.axline((CONTROL_START_IDX, 0), (CONTROL_START_IDX, 1),
                  color='black', linestyle='--', alpha=0.5, label='Control Start')
        ax.legend()
        ax.set_title(f"{title} | Step: {self.step_idx}")
        ax.set_xlabel(axis_labels[0])
        ax.set_ylabel(axis_labels[1])

    def compute_cost(self, last_step_only=False) -> Dict[str, float]:
        if last_step_only:
            target = np.array(self.target_lataccel_history)[self.step_idx]
            pred = np.array(self.current_lataccel_history)[self.step_idx]
        else:
            target = np.array(self.target_lataccel_history)[
                CONTROL_START_IDX:len(self.data)]
            pred = np.array(self.current_lataccel_history)[
                CONTROL_START_IDX:len(self.data)]

        lat_accel_cost = np.mean((target - pred)**2) * 100
        if last_step_only:
            jerk_cost = (self.current_lataccel -
                         self.current_lataccel_history[self.step_idx - 1])**2 * 100
        else:
...
def run_rollout(data_path, controller_type, model_path, debug=False):
    ...
    controller = importlib.import_module(
        f'controllers.{controller_type}').Controller()
    sim = TinyPhysicsSimulator(tinyphysicsmodel, str(
        data_path), controller=controller, debug=debug)
    return sim.rollout(), sim.target_lataccel_history, sim.current_lataccel_history


def generate_rollout(data_path, model_path, debug=False):
    tinyphysicsmodel = TinyPhysicsModel(model_path, debug=debug)
    sim = TinyPhysicsSimulator(tinyphysicsmodel, str(
        data_path), debug=debug)
    return sim
...
if __name__ == "__main__":
    ...
    if data_path.is_file():
        ...
        run_rollout_partial = partial(
            run_rollout, controller_type=args.controller, model_path=args.model_path, debug=False)
        files = sorted(data_path.iterdir())[:args.num_segs]
        results = process_map(run_rollout_partial, files,
        ...
        for cost in costs_df.columns:
            plt.hist(costs_df[cost], bins=np.arange(
                0, 1000, 10), label=cost, alpha=0.5)
        plt.xlabel('costs')
